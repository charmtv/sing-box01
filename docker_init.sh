...
if ! [[ "${START_PORT:-}" =~ ^[0-9]+$ ]] || (( START_PORT < 100 || START_PORT > 65520 )); then
  warning "START_PORT 必须是 100-65520 之间的整数"
  exit 1
fi

if [ -z "${SERVER_IP:-}" ]; then
  warning "SERVER_IP 不能为空"
  exit 1
fi
