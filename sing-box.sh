...
check_cdn() {
  ...
  local RAW_URL='https://raw.githubusercontent.com/charmtv/sing-box01/main/sing-box.sh'
...
get_sing_box_version() {
  ...
  local FORCE_VERSION=$(wget --no-check-certificate --tries=2 --timeout=3 -qO- ${GH_PROXY}https://raw.githubusercontent.com/charmtv/sing-box01/refs/heads/main/force_version | sed 's/^[vV]//g; s/\r//g')
...
bash <(wget --no-check-certificate -qO- https://raw.githubusercontent.com/charmtv/sing-box01/main/sing-box.sh) \$@
EOF
...
# 传参处理1: 把所有的 = 变为空格，但保留 =" ，以免破坏带等号的 JSON 字符串值。
